...
<div style="max-width: 700px; margin: 0 auto; padding: 20px;">
    <div style="margin-bottom: 30px;">
        ...
        <p style="margin: 8px 0 0; color: #6b7280;">Send a direct message to {{ $serviceProvider->full_name }}.</p>
    </div>
    ...
    <div style="background-color: #fff; border-radius: 12px; padding: 24px; border: 1px solid #e5e7eb; box-shadow: 0 1px 4px rgba(0,0,0,0.05); margin-bottom: 24px;">
        <div style="display: flex; gap: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 220px;">
                <div style="width: 100%; aspect-ratio: 4 / 3; background-color: #f3f4f6; display: flex; align-items: center; justify-content: center; border-radius: 8px; color: #6b7280; font-weight: 700;">
                    Service Provider
                </div>
            </div>
            <div style="flex: 2; min-width: 220px;">
                <h2 style="margin: 0 0 10px; color: #111827;">{{ $serviceProvider->full_name }}</h2>
                <p style="margin: 0 0 12px; color: #4b5563; line-height: 1.6;">{{ $serviceProvider->description }}</p>
                <div style="font-weight: 700; color: #16a34a; font-size: 18px;">{{ $serviceProvider->service_category }}</div>
                <div style="margin-top: 12px; color: #4b5563;">Location: {{ $serviceProvider->location }}</div>
            </div>
        </div>
    </div>
    ...
    @if($showReviewPrompt)
        <div id="reviewPrompt" style="position: relative; background-color: #fff7ed; border: 1px solid #fdba74; border-radius: 14px; padding: 20px; margin-bottom: 24px; box-shadow: 0 10px 24px rgba(0,0,0,0.08);">
            ...
            <form method="POST" action="{{ route('services.reviews.store', $serviceProvider) }}" style="display:grid; gap:12px;">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <textarea id="review_text" name="review" rows="3" placeholder="Share your experience, or leave it blank..." style="width:100%; padding:12px; border:1px solid #d1d5db; border-radius:8px;"></textarea>
                    @error('review')
                        <p style="margin-top:8px; color:#dc2626; font-size:14px;">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <form method="POST" action="{{ route('messages.provider.store', $serviceProvider) }}" enctype="multipart/form-data" style="background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 24px;">
        @csrf
        ...
        <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
            ...
            <a href="{{ route('services.index') }}" style="text-decoration: none; color: #374151;">← Back to services</a>
        </div>
    </form>
</div>
...
@if($showReviewPrompt)
    <script>
        (function () {
            ...
            const storageKey = 'provider_review_prompt_dismissed_{{ $serviceProvider->id }}_{{ auth()->id() }}';
